...
Vector<Vector> parameters = updatesub.getParameters(subTablenames, pid, centerid, modid, subTableIdName, connection, paramConn);

//Retrieve the aliquot information
ResultSet rsAliquots = updatesub.getAliquotInfo(pid, centerid, modid, dbn, statement);

Vector<Vector> labelParameters = updatesub.getLabelInfo(rsAliquots, parameters);
//Vector<Vector> labelParameters = updatesub.getLabelInfo(parameters);

String dateValue = updatesub.getDateValue(parameters);

//int labelNum = labelParameters.size();
...
logger.debug("('" + username + "') - modality/print_label.jsp (modality = '" + modality + "')");


%>

<h1><%=dbn%> Record <%=centerid%>-<%=pid%> Labels</h1>
...
<h2><%=dbn%> Print Biomaterial Record <%=modid%></h2>

<form name="f1"  action="./jsp/search/export_pdf_direct.jsp?dbid=<%=dbid%>&dbn=<%=dbn%>&pid=<%=pid%>&centerid=<%=centerid%>&modality=<%=modality%>&modid=<%=modid%>" method="POST">

<table border="1px" cellpadding="5">
        <%
    String printLabelHtml = updatesub.getPrintLabelHtml(labelParameters, lineColour, dateValue, modid, dbn, statement);
    %>
    <%= printLabelHtml %>

</table>

<input type="submit" name="print_labels_a4" value="Print Labels (A4)"/>
<input type="submit" name="print_labels" value="Print Labels"/>
</form>
