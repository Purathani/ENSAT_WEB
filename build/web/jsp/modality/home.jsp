...
logger.debug("('" + username + "') - modality/home.jsp (modality = '" + modality + "')");
...
<p><a href="./jsp/modality/create/create_view.jsp?dbid=<%=dbid%>&dbn=<%=dbn%>&pid=<%=pid%>&centerid=<%=centerid%>&modality=<%=modality%>">Create New <%=pageTitle%> Record</a></p>

<table border="1px" cellpadding="10" width="100%">
    
        <%        
        String assocInfoListHtml = updatesub.getAssocInfoHtml(dbid,dbn,centerid,pid,connection,modality);
    %>  
    <%= assocInfoListHtml %>
</table>
