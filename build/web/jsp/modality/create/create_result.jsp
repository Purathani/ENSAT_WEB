...
logger.debug("('" + username + "') - modality/create_result.jsp (modality = '" + modality + "')");
...
<h1>Record has been successfully created</h1>
...
if(modality.equals("biomaterial")){
    %>
<h3><a href="./jsp/modality/read/print_label.jsp?dbid=<%=dbid%>&dbn=<%=dbn%>&pid=<%=pid%>&centerid=<%=centerid%>&modality=<%=modality%>&modid=<%=nextId%>">Print Labels <%=centerid%>-<%=pid%> (<%=nextId%>)</a></h3>
<%
}
